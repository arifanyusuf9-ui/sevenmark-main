...
export function VaporizeAnimationText({
    ...
    const getTextCoords = (canvasWidth: number, canvasHeight: number) => {
        let x = canvasWidth / 2
        let y = canvasHeight / 2

        if (textX !== undefined) {
            if (typeof textX === 'number') x = textX
            else if (textX.endsWith('%')) x = (parseFloat(textX) / 100) * canvasWidth
        }
        if (textY !== undefined) {
            if (typeof textY === 'number') y = textY
            else if (textY.endsWith('%')) y = (parseFloat(textY) / 100) * canvasHeight
        }
        return { x, y }
    }
    ...
    useEffect(() => {
        ...
        const createParticles = (text: string) => {
            ...
            const sampleRate = 3 // Slightly less dense for a more 'dotted' and elegant look
        ...
        const drawGlow = (x: number, y: number, radius: number, glowColor: string) => {
            ...
            gradient.addColorStop(0, glowColor.replace(/[\d.]+\)$/, '0.2)')) // Much softer glow
            ...
            switch (animationState) {
                case "vaporizing": {
                    ...
                    particlesRef.current.forEach(particle => {
                        ...
                        if (shouldVaporize) {
                            ...
                            // Particles roam freely beyond canvas bounds
                            particle.previousX = particle.x
                ...
                case "waiting": {
                    particlesRef.current.forEach(particle => {
                        ctx.fillStyle = particle.color
                        ctx.fillRect(particle.x, particle.y, 2, 2)
                        if (config.effects.glow) drawGlow(particle.x, particle.y, 2, particle.color)
                    })
                    break
                }
                default: {
