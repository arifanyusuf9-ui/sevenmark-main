...
const postFragmentShader = `
  ...
  void main() {
    ...
    final_color.rgb = pow(final_color.rgb, vec3(0.9)); // Slight gamma adjustment
    final_color.rgb = mix(final_color.rgb, final_color.rgb * vec3(1.1, 0.95, 1.05), 0.3); // Color tint
    ...
    const handleClick = useCallback((e: React.MouseEvent) => {
        ...
        particlesRef.current.forEach(particle => {
            ...
            if (distance < 120) {
                ...
                particle.vx += Math.cos(pAngle) * force * 90;
                particle.vy += Math.sin(pAngle) * force * 90;
            }
        });
    }, [particleSize]);
