...
const HeroShowcase = () => {
    ...
    return (
        ...
        >
            <div className="flex items-center items-end gap-4 h-32">
                {showcaseImages.map((img, i) => (
                    <motion.div
                        key={i}
                        onClick={() => setActiveIndex(i)}
                        animate={{
                            scale: activeIndex === i ? 1.25 : 0.8,
                            opacity: activeIndex === i ? 1 : 0.4,
                        }}
                        transition={{ duration: 0.6, ease: [0.23, 1, 0.32, 1] }}
                        className={cn(
                            "rounded-2xl border backdrop-blur-md flex items-center justify-center p-3 cursor-pointer overflow-hidden relative group",
                            i === activeIndex
                                ? 'h-24 w-24 border-primary bg-black/40 shadow-[0_0_30px_rgba(212,175,55,0.2)]'
                                : 'h-16 w-16 border-white/5 bg-white/5'
                        )}
                    >
                        <img
                            src={img}
...
const CinematicHero = () => {
    return (
        <section className="relative h-screen w-full bg-black flex flex-col items-center justify-center overflow-hidden">
            ...
            <div className="relative z-10 w-full max-w-7xl px-8 flex items-center justify-between h-full">
                ...
                <motion.div
                    ...
                    className="max-w-[340px] space-y-6"
                >
                    <div className="space-y-2">
                        <h2 className="font-heading text-4xl text-white font-light tracking-wide leading-tight">
                            Curated Craft,<br />
                ...
                >
                    <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 w-[260px] shadow-2xl overflow-hidden relative group">
                        <div className="absolute top-0 right-0 w-32 h-32 bg-primary/5 blur-3xl -mr-16 -mt-16" />
                        <p className="text-[10px] uppercase tracking-[0.2em] text-white/40 mb-4 font-bold">Box Components</p>
                        <div className="flex items-start gap-4 mb-6">
                            <span className="text-5xl font-heading text-white font-light">3</span>
                            <div className="flex flex-col gap-1">
                                <span className="text-[10px] uppercase tracking-[0.1em] text-white font-bold leading-none">Premium</span>
                                <span className="text-[14px] uppercase tracking-[0.2em] text-white leading-none">Items</span>
                            </div>
                        </div>
                        <ul className="space-y-3">
                            <li className="flex items-center gap-3">
                                <div className="h-1 w-1 bg-primary rotate-45" />
                                <span className="text-[10px] uppercase tracking-[0.2em] text-white/70 font-bold">Notebook, Pen, Tumbler</span>
                            </li>
                        </ul>
                    </div>
                </motion.div>
            ...
            <div className="absolute bottom-12 left-0 right-0 px-8 flex items-end justify-end z-20">
                <HeroShowcase />
            </div>
